...
def hard_reset_environment(seed):
    ...
    torch.backends.cudnn.deterministic = False # This can be turned to true incase anything breaks (NaNs,not learning) 
...
class ImageDataset(Dataset):
    def __init__(self,dataset_name="PetImages",split="train"):
        ...
        images = np.load(images_path, mmap_mode="r")    #image datasets are large, use memmap to poll into memory only when required
        ...
        ]) if device.type != "cpu" else None #without a gpu it might take a while...
...
def fit(model, device, train_dataset, epochs=2, log_callback=print):
    ...
    for i in range(epochs):
        ...
        log_callback(f"epoch {i+1} / {epochs}: total_loss/n = {total_loss/len(train_loader)} : lr: {scheduler.get_last_lr()[0]:.6f}")
        scheduler.step()
...
if __name__ == "__main__":
    ...
    EPOCHS = 25
    SEED = 42
    DATASET_NAME = "this should match the name of your processed dataset without file extensions"
